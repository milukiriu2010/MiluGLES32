...
// -----------------------------------------------------------
class A12Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(球体)
    ...
    // VAO(板ポリゴン)
    private val vaoBoard = ES32VAOIpn()

    // シェーダA
    private val shaderA = ES32a12ShaderA(ctx)
    // シェーダB
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        shaderA.draw(vaoSphere,matM,matMVP,matN)

        // トーラス
        shaderA.draw(vaoTorus,matM,matMVP,matN)
