...
// -----------------------------------
class W67Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(トーラス)
    ...
    // シェーダ(メイン)
    private val shaderScreen = W67ShaderScreen(ctx)
    // シェーダ(ズームブラー)
    private val shaderZoomBlur = W67ShaderZoomBlur(ctx)
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        // 板ポリゴンの描画
        shaderZoomBlur.draw(vaoBoard,matVP,0,u_strength,renderW.toFloat())
    }
