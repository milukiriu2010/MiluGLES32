...
// -------------------------------------
class W71Renderer(ctx: Context): MgRenderer(ctx) {
    // 描画オブジェクト(球体)
    ...
    // シェーダ(点のレンダリングを行う)
    private val shaderPoint = W71ShaderPoint(ctx)
    // シェーダ(テクスチャへの描きこみを行う)
    private val shaderMapping = W71ShaderMapping(ctx)
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        ...
        MyGLES32Func.createFrameBuffer(16,16,0,frameBuf,depthRenderBuf,frameTex)
    }
